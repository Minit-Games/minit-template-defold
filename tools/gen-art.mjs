...
// when you bring your own art -- only the atlas entry names matter to the game.
...
const OUT = join(dirname(fileURLToPath(import.meta.url)), '..', 'assets', 'atlas', 'img');
...
// Emitted here so a new sprite cannot be added without the atlas learning about
// it. Each entry becomes a one-frame flipbook named after the file.
const atlas = Object.keys(SPRITES).sort().map(n =>
  `images {\n  image: "/assets/atlas/img/${n}.png"\n  sprite_trim_mode: SPRITE_TRIM_MODE_OFF\n}`).join('\n');
await writeFile(join(OUT, '..', 'game.atlas'),
  `${atlas}\nmargin: 2\nextrude_borders: 2\ninner_padding: 0\n`);
