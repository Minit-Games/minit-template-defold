...
// game, and each one also gets a .sound component written beside it.
...
// in the repo and the bundle. music.wav/.sound come from gen-music.mjs.
const keep = new Set([...Object.keys(BANK), 'music']);
for (const file of await readdir(OUT)) {
  const base = file.replace(/\.(wav|sound)$/, '');
...
for (const [name, make] of Object.entries(BANK)) {
  ...
  // A .sound component per clip, so the game can address them by name.
  await writeFile(join(OUT, `${name}.sound`),
    `sound: "/assets/sound/${name}.wav"\nlooping: 0\ngroup: "master"\ngain: 1.0\npan: 0.0\nspeed: 1.0\nloopcount: 0\n`);
  total += wav.length;
  names.push(name);
